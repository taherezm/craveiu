    ...
    <div className="mx-auto max-w-3xl px-4 py-8 sm:px-6">
      ...
      <div className="mb-6 flex items-start justify-between">
        ...
        <div className="flex items-center gap-2">
          <button
            onClick={refresh}
            disabled={isLoading}
            className="flex h-9 w-9 items-center justify-center rounded-full border border-gray-200 dark:border-zinc-700 text-gray-500 dark:text-zinc-400 transition-colors hover:bg-gray-50 dark:hover:bg-zinc-800 disabled:opacity-40"
            title="Refresh menus"
          >
          ...
          <Link
            href="/preferences"
            className="flex h-9 w-9 items-center justify-center rounded-full border border-gray-200 dark:border-zinc-700 text-gray-500 dark:text-zinc-400 transition-colors hover:bg-gray-50 dark:hover:bg-zinc-800"
      ...
      <div className="relative mb-6">
        ...
        <input
          ...
          className="w-full rounded-full border border-gray-200 dark:border-zinc-700 bg-white dark:bg-zinc-800 py-2.5 pl-10 pr-4 text-sm text-gray-900 dark:text-white outline-none transition-colors placeholder:text-gray-400 dark:placeholder:text-zinc-500 focus:border-[#990000] focus:ring-2 focus:ring-[#990000]/10"
        />
      </div>

      {/* Error state */}
      {error && (
        <div className="mb-6 flex items-start gap-3 rounded-xl border border-red-100 dark:border-red-800 bg-red-50 dark:bg-red-900/30 p-4 text-sm text-red-700 dark:text-red-400">
      ...
      {!isLoading && !hasPreferences && activeFilters.length === 0 && (
        <div className="mb-6 rounded-2xl border border-dashed border-gray-200 dark:border-zinc-700 bg-gray-50 dark:bg-zinc-900 p-6 text-center">
          <p className="text-sm font-medium text-gray-700 dark:text-zinc-200">No preferences set yet.</p>
      ...
      <div className="space-y-3">
        ...
        {!isLoading && filteredRankings.length === 0 && searchQuery && (
          <div className="rounded-2xl border border-gray-100 dark:border-zinc-800 bg-gray-50 dark:bg-zinc-900 p-8 text-center">
            <p className="text-sm text-gray-500 dark:text-zinc-500">No results for &quot;{searchQuery}&quot;</p>
          </div>
        )}
      </div>
    </div>
